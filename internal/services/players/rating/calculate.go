...
func CalculatePlayerScoresv2(players []models.PlayerStats) ([]models.PlayerScore, error) {
	...
	for i, playerStats := range players {
		...
		playerScores[i] = models.PlayerScore{Id: playerStats.Id, NewRating: newRating, RatingChange: int(math.Floor(ratingDelta))}
	}
